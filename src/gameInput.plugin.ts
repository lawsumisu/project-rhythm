...
export class InputHistory {
  ...
  public update(input: Phaser.Input.InputPlugin): void {
    ...
    const gamepad = this.pad && gamepadInput && !!gamepadInput[this.pad] && gamepadInput[this.pad];
    Object.keys(this.mapping).forEach((input: GameInput) => {
      for (const config of this.mapping[input]!) {
        if (
          (isKeyboard(config) && keyboard && keyboard.addKey(config.key).isDown) ||
  ...
  public toString(): string {
    ...
    return Array.from(inputs).sort().join(',');
  }

  public clear(): void {
    this.inputHistory = new RingBuffer(this.historyLength);
    for (let i = 0; i < this.historyLength; i++) {
      this.inputHistory.push(new Set());
    }
  }
...
export class GameInputPlugin extends Phaser.Plugins.ScenePlugin {
  ...
  private histories: InputHistory[] = [
    new InputHistory(
      Object.keys(GameInputPlugin.defaultGamePadInputs).reduce(
        (acc, key: Exclude<keyof InputMap, '↖' | '↗' | '↙' | '↘'>) => {
          acc[key] = [...GameInputPlugin.defaultGamePadInputs[key], ...GameInputPlugin.defaultKeyboardInputs[key]];
          return acc;
        },
        {} as InputMap
      ),
  ...
  public boot(): void {
    this.systems!.events.on('start', this.onSceneStart)
